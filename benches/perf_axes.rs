...
fn main() {
    ...
    let threads: usize = std::env::var("RLA_PERF_THREADS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(8);
    let cap_mb: f64 = std::env::var("RLA_PERF_MEM_CAP_MB")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(6000.0);
    ...
    for &(group, name) in MATS {
        ...
        if (est.transient_peak_bytes.max(est.mf_transient_peak_bytes) as f64 / 1e6) > cap_mb {
            eprintln!("skip {name}: over mem cap");
            continue;
        }
        ...
        let ll = timed(
            &sym,
            &a,
            &base.clone().with_method(FactorMethod::LeftLooking),
            3,
        );
        let mf = timed(
            &sym,
            &a,
            &base.clone().with_method(FactorMethod::Multifrontal),
            3,
        );
        let rl = timed(
            &sym,
            &a,
            &base.clone().with_method(FactorMethod::RightLooking),
            3,
        );
        ...
        let solve_str = rslab::factor_sparse_ldlt_with(&a, &base)
            .ok()
            .map(|f| {
                let nrhs = 256usize;
                let b: Vec<C> = (0..a.n * nrhs)
                    .map(|k| C::new((k % 11) as f64 - 5.0, 0.2))
                    .collect();
                let t = Instant::now();
        ...
        let comp_str = rslab::factor_sparse_ldlt_with(&a, &base)
            .ok()
            .and_then(|f| {
                let full = 8 * (f.l_col_ptr.len() + f.l_row_idx.len() + f.perm.len());
                CompressedLdltFactors::from_factors(f).map(|c| {
                    format!(
                        "{:.0}->{:.0}KB",
                        full as f64 / 1024.0,
                        c.index_bytes() as f64 / 1024.0
                    )
                })
            })
            .unwrap_or_else(|| "-".into());
        ...
        let fmt = |o: Option<(f64, f64)>| {
            o.map(|(m, mb)| format!("{m:>8.1} /{mb:>6.0}"))
                .unwrap_or_else(|| "     -    ".into())
        };
        println!(
            "{:<14} {:>8} {:>10} | {:>18} | {:>18} | {:>18} | {:>14} | {:>16}",
...
fn main() {
    eprintln!("perf_axes requires --features matgen-download");
}
