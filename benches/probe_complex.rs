...
fn main() {
    ...
    println!("{:<28}{:>8}{:>10}{:>12}{:>14}", "matrix", "type", "n", "nnz", "max|imag|");
    println!("{}", "-".repeat(74));
    for gn in list.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()) {
        let Some((group, name)) = gn.split_once('/') else { continue };
        let path = match rslab::matgen::download::fetch(group, name) {
            Ok(p) => p,
            Err(e) => {
                println!("{:<28}{:>8}  fetch/skip: {}", name, "-", e.lines().next().unwrap_or(""));
                continue;
            }
        };
        match rslab::read_mtx_any(&path) {
            Ok(rslab::MtxLoaded::Symmetric(a)) => {
                ...
                if mi > 0.0 { complex += 1; }
                sym += 1;
                println!("{:<28}{:>8}{:>10}{:>12}{:>14.2e}", name, "sym", a.n, a.values.len(), mi);
            }
            Ok(rslab::MtxLoaded::General(a)) => {
                let mi = a.values.iter().map(|v| v.im.abs()).fold(0.0, f64::max);
                if mi > 0.0 { complex += 1; }
                gen += 1;
                println!("{:<28}{:>8}{:>10}{:>12}{:>14.2e}", name, "general", a.n, a.values.len(), mi);
            }
