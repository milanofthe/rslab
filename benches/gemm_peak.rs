...
fn bench(m: usize, n: usize, k: usize, par: gemm::Parallelism, label: &str) {
    ...
    unsafe {
        gemm::gemm(
            m, n, k, c.as_mut_ptr(), m as isize, 1, true,
            a.as_ptr(), m as isize, 1, b.as_ptr(), k as isize, 1,
            C::new(1.0, 0.0), C::new(-1.0, 0.0), false, false, false, par,
        );
    }
    let reps = 5;
    let t = Instant::now();
    ...
    println!("  {label:28} {m}x{n}x{k}  {:.2} ms  {gflops:.1} Gflop/s", s * 1e3);
}
...
fn main() {
    ...
    for &k in &[64usize, 128, 256, 512] {
        bench(2000, 2000, k, gemm::Parallelism::Rayon(0), &format!("rank-{k}"));
    }
