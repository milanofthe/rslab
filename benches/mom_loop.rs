...
fn bench_file(path: &std::path::Path) {
    ...
    let exact = LuSolver::factor(&a, &SolverSettings::preconditioner(1e-10)).unwrap();
    run_precond("f64 LU (no drop)", &a, &b, exact.factor_nnz(), 16, &exact);

    // f32 LU - half the factor memory.
    let f32lu = LowPrecisionLu::factor(&a, &SolverSettings::preconditioner(1e-10)).unwrap();
    ...
    let ilu = LuSolver::factor(
        &a,
        &SolverSettings::preconditioner(1e-10).with_drop_tol(1e-2),
    )
    .unwrap();
