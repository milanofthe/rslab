...
and mid sizes outright ($0.23\times$--$0.77\times$ \rT's factor time in
...
the tables (Accelerate $1.3$--$1.35\times$ slower there; its time scales at
$\alpha\approx1.67$ vs.\ $1.38$). The circuit comparison is one-shot
analyze+factor+solve against the strictly sequential KLU default; KLU's
numeric-only refactor sweeps and the parallel per-block factor are not
exercised. On
...
unsym ($\LU$ path) & $0.27\times$ & $0.29\times$ & 60 \\
