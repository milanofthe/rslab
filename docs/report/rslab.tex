...
residuals ($\sim10^{-15}$) on every size. An opt-in parallel per-block factor
(independent BTF blocks on a scoped pool, bit-identical to the sequential result)
adds another \numrange{2.2}{3.1}$\times$.
...
(e.g.\ \SI{179}{ms} vs.\ \SI{152}{ms} at $n=200$k) and the refactor within
\numrange{1.15}{1.4}$\times$; the opt-in bit-identical parallel per-block
factor is \numrange{1.5}{2.3}$\times$ \emph{faster} than SuiteSparse KLU
across the range (\SI{81}{ms} at $n=200$k). Both reach $\sim10^{-15}$
residuals.
