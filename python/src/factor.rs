...
//! the matrix it factored) over the core's `Factorization` trait the three
//! solvers share, wrapped per scalar field in an enum; the Python classes are
...
pub trait Direct<T: Field>: Factorization<T> + Send + Sync + Sized {
...
impl<T: Field, S: Direct<T>> Pair<T, S> {
        ...
        let x = py
            .allow_threads(|| {
                if policy.max_steps > 0 {
                    self.s
                        .solve_refined(&self.a, &bvec, &policy)
                        .map(|(x, _)| x)
                } else {
                    self.s.solve(&bvec)
                }
    ...
    pub fn solve_many(&self, py: Python<'_>, b: &Bound<'_, PyAny>) -> PyResult<PyObject> {
        ...
        let x = py
            .allow_threads(|| self.s.solve_many(&cm, nrhs))
            .map_err(map_err)?;
        array2(py, &x, n, nrhs)
    }
