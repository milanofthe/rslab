...
3. **exact ND bakeoff** on large systems (n >= 10k, >= 5e9 flops): re-analyze
   ...
   regression in exact fill or transient peak (both paths: LDL^T and now LU);
...
* **Speedup must be measured in the flop-dense regime.** The old 24^3 f64
  grid measured 1.37x@12 (bandwidth-bound + too small), which made the cost
  model pick `Fixed(2)` for helmholtz -> 1609 ms (2.1x worse than @8). Now:
  complex 32^3 grid, **MetisND-ordered** (measure what the machine *can* do
  on a parallelizable tree; per-matrix structure is the cost model's job via
  the critical-path floor), warm-up factor before timing -> 2.38x@12.
...
## Measured result (helmholtz 40^3 complex, warm best-of-3)
...
is deliberately **no auto-router** between LDL^T / LU / KLU: rslab is a
...
* **`btf = on`**: off costs 48x factor time at 200k (13.5-20 s vs 280 ms)
  and 5.7x fill (52 M vs 9.2 M). On the misscaled variant off is 20x+.
