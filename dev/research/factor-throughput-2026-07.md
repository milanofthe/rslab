...
* **Ordering ~ 3-4x of the gap.** Our own MetisND on this matrix: fill
  ...
  the critical path (ND: 1.32e10 -> Amdahl ceiling 3.4x instead of 1.6x)
  and, for free, the remaining 1.18x solve gap.
* **Rate ~ 2-3x.** The `gemm` crate is NOT the ceiling (measured c64 peak
...
node-execution overlap ~1.4x (measured: sum node-wall / wall), (b) per-update
...
1. **rslab-metis separator quality on 3D meshes**, **DONE 2026-07**
   ...
   (cumulative 1552 -> 774 = 2.0x). Note: AUTO still routes to AMF
