...
(exact scalar nnz(L) 20.9 M -> 14.0 M on the 40^3 grid; helmholtz probe
...
Reference pattern: 40^3 7-point grid (the `helmholtz_64000` pattern),
...
symbolic elimination on 3^3-6^3 grids with random permutations.

## The evidence chain

| ordering | exact nnz(L) 40^3 |
|---|---|
...
1. **Separator size is NOT the lever.** The old pipeline's top-level
   separator on 40^3 was exactly 1600 = the perfect 40x40 plane, and
...
2. **The fill excess is distributed, not top-heavy.** Fill by
   ...
   both), the loss is 2-4x in every band below it, i.e. in how the
...
40^3 grid exact fill 14.00 M (seeds 2-4: 14.3/14.5/14.9 M); flops
2.29e10 -> 1.53e10. Scaling: 24^3 1.88 -> 1.53 M (-19 % vs AMD);
56^3 108.2 (AMD) -> 63.8 M (-41 %).
...
-> 1216 (kernel fixes) -> 774 ms (this work) = **2.0x**; MKL PARDISO is
~170-220 ms, so the head-to-head gap is now ~3.5-4.5x (was 8.8x).
